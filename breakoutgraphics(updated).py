...
#         maybe_obj4 = self.window.get_object_at(point4_x, point4_y)
        ...
        maybe_obj5 = self.window.get_object_at(point5_x, point5_y)
        maybe_obj6 = self.window.get_object_at(point6_x, point6_y)
...
#         if (self.ball.y + BALL_RADIUS * 2) - (b - PADDLE_OFFSET) > 0:
#             if maybe_obj11 is not None:
#                 self.__dx = -self.__dx
#             if maybe_obj12 is not None:
#                 self.__dx = -self.__dx
...
#                 self.__dy = -self.__dy
#             if maybe_obj6 is not None:
#                 self.__dy = -self.__dy
#             print(maybe_obj1 is None)
#             if maybe_obj1 is not None and maybe_obj1 is not self.paddle:
#                 self.__dy = -self.__dy
#             if maybe_obj2 is not None and maybe_obj2 is not self.paddle:
#                 self.__dy = -self.__dy
#             if maybe_obj3 is not None and maybe_obj3 is not self.paddle:
#                 self.__dy = -self.__dy
#             if maybe_obj4 is not None and maybe_obj4 is not self.paddle:
#                 self.__dy = -self.__dy
        print(maybe_obj5 is not None)
        print(maybe_obj5 is not self.paddle)
        if maybe_obj5 is not None and maybe_obj5 is not self.paddle:
            self.__dy = -self.__dy
        if maybe_obj6 is not None and maybe_obj6 is not self.paddle:
            self.__dy = -self.__dy
        """
    ...
    def remove_brick(self):
        ...
        # if maybe_obj7 is not None and maybe_obj7 is not self.paddle:
        #     self.window.remove(maybe_obj7)
        #     self.__dx = -self.__dx
        # if maybe_obj8 is not None and maybe_obj8 is not self.paddle:
